...
const int NPR_CELSHADING = 17;
const int NPR_XTOON = 18;
const int NPR_GOOCH = 19;
const int NPR_CEL_GOOCH = 20;
const int NPR_HATCH = 21;

uniform vec3 SunDirTo;
uniform vec3 SunE0;

uniform sampler2D HatchMap[4];
uniform sampler2D XToonMap[2];
...
vec3 Toon(vec3 N, vec3 L) {
    float t = CelShade(N, L);
    return Material.c_d * t;
}

vec3 XToon(vec3 N, vec3 L) {
    return vec3(1.0);
}

vec3 Hatch(vec3 N, vec3 L) {
    vec2 st = vec2(gl_FragCoord.xy) / vec2(ScreenWidth, ScreenHeight);
    float t = CelShade(N, L);
    //return t * vec3(st, 0.0);
    if (t < 0.25) return texture(MapKd, st).rgb;
    if (t < 0.50) return texture(HatchMap[1], st).rgb;
    if (t < 0.75) return texture(HatchMap[2], st).rgb;
    if (t < 1.00) return texture(HatchMap[3], st).rgb;
    return vec3(1.0);
}
...
void main() {
    ...
    case NPR_CELSHADING:
        ...
        break;
    case NPR_XTOON:
        color = XToon(Fragment.N, L);
        break;
    case NPR_GOOCH:
        color = Gooch(Fragment.N, L);
        break;
    case NPR_CEL_GOOCH:
        color = CelShade(Fragment.N, L) * Gooch(Fragment.N, L);
        break;
    case NPR_HATCH:
        color = Hatch(Fragment.N, L);
        break;
    ...
    if (GBufferOutputType >= NPR_CELSHADING) {
