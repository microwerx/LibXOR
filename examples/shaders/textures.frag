...
uniform sampler2D MapKsRoughness;
...
{
	vec3 map = 2.0 * texture(MapNormal, texcoord).rgb - 1.0;
...
void PrepareForShading() {
    ...
    if (MapNormalMix > 0.0) {
        vec3 bumpN = PerturbNormal(N, V, vTexcoord.st);
        N = MapNormalMix * bumpN + (1.0 - MapNormalMix) * N;        
    }
    Fragment.N = normalize(N);
    Fragment.V = V;
...
float ReadValue(float mixAmount, sampler2D tex, float x) {
    if (mixAmount > 0.0) {
        vec3 color = 1.0 - texture(tex, vTexcoord.st).rgb;
        float t = (color.r + color.g + color.b) / 3.0;
        return (1.0 - mixAmount) * x + mixAmount * t;
    }
    return x;
}
...
void main() {
    ...
    float specularRoughness = ReadValue(MapKsRoughnessMix, MapKsRoughness, KsRoughness);
